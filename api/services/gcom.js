    ...
    for( var i = 0;i<propertyrelated.length; i++ ) {
        ...
        if ( children.length ) {
            ...
            for (var j = 0; j < children.length; j++) {
                 ...
                 subNormsMap = self.isResetKey('seckillingstock',subNormsMap);
                 //console.log('subNormsMap: check it out: ',subNormsMap);
                 ...
                 subgoods.set('seckillingsell', subNormsMap.get('seckillingsell') || 0);
                 subgoods.set('seckillingprice', subNormsMap.get('seckillingprice') || 0);
                 subgoods.set('seckillingstock', subNormsMap.get('seckillingstock') || 0);

                 goods.set('stock', goods.get('stock') + parseInt(subNormsMap.get('stock')));
                 goods.set('seckillingstock', parseInt(goods.get('seckillingstock')) + parseInt(subNormsMap.get('seckillingstock')));
            ...
            subObjMap = self.isResetKey('seckillingstock',subObjMap);
            ...
            subgoods.set('seckillingsell', subObjMap.get('seckillingsell') || 0);
            subgoods.set('seckillingprice', subObjMap.get('seckillingprice') || 0);
            subgoods.set('seckillingstock', subObjMap.get('seckillingstock') || 0);
            subgoods.set('propertyrelated',self.hashToString(temp));
            ...
            goods.set('seckillingstock', parseInt(goods.get('seckillingstock')) + parseInt(subgoods.get('seckillingstock')));
            //console.log('stock: check it out: ', goods.get('stock'));
            //console.log('sku: check it out: ', subgoods.get('sku'));
...
gcom.goodsStruct = function() {
    return {  
        ...
        valArray: {
            ...
            seckillingtime:true,
            pricepromotion:true,
            seckillingstock:true,
            propertyrelated:true,
            homenormalprice:true,
            seckillingprice:true,
            propertyvaluelist:true,
            homeseckillingprice:true,
        },

    }  
};

gcom.hashMerchantGoods = function(goods) {
    ...
    var goodsStruct = self.goodsStruct();
    keyArray.isdelete = 0;
    keyArray.tablename = gcom.getMysqlTable(TAB_M_GOODS,goods.get('storeid'));
    keyArray.parentid = goods.get('parentid');
    keyArray.storecategoryid = goods.get('storecategoryid');
    keyArray.goodsseries = goods.get('goodsseries');
    keyArray.status = goods.get('status');
    keyArray.userid = goods.get('userid');
    keyArray.storeid = goods.get('storeid');
    keyArray.name = goods.get('name');
    keyArray.type = goods.get('type');
    keyArray.sku = goods.get('sku');
    keyArray.updatedAt = new Date().getTime();
    keyArray.createdAt = new Date(goods.get('createdAt')).getTime();
    ...
    valArray.price = goods.get('price');
    valArray.deposit = goods.get('deposit');
    valArray.limited = goods.get('limited');
    valArray.premoneey = goods.get('premoneey');
    valArray.detailbody = goods.get('detailbody');
    valArray.pricepoint = goods.get('pricepoint');
    valArray.attachment = goods.get('attachment');
    valArray.propertypic = goods.get('propertypic');
    valArray.isseckilling = goods.get('isseckilling');
    valArray.imagedefault = goods.get('imagedefault');
    valArray.seckillingsell = goods.get('seckillingsell');
    valArray.seckillingtime = goods.get('seckillingtime');
    valArray.pricepromotion = goods.get('pricepromotion');
    valArray.seckillingstock = goods.get('seckillingstock');
    valArray.propertyrelated = goods.get('propertyrelated');
    valArray.homenormalprice = goods.get('homenormalprice');
    valArray.seckillingprice = goods.get('seckillingprice');
    valArray.propertyvaluelist = goods.get('propertyvaluelist');
    valArray.homeseckillingprice = goods.get('homeseckillingprice');

    valArray.keywords = "";
    valArray.imgfocus = "";
    valArray.recommend = "";
    valArray.reserve10 = goods.get('reserve10') || "";
    valArray.presubtitle = goods.get('presubtitle') || "";
    valArray.presaleflow = goods.get('presaleflow') || "";
    valArray.seckillingflow = goods.get('seckillingflow') || "";
    valArray.presaledescript = goods.get('presaledescript') || "";
    valArray.seckillingexplain = goods.get('seckillingexplain') || "";
    valArray.presaleflowdescript = goods.get('presaleflowdescript') || "";
    valArray.precustomerserivice = goods.get('precustomerserivice') || "";
    valArray.seckillingdescription = goods.get('seckillingdescription') || "";

    

    var keysArray = _.toPairs(keyArray);
    var valsArray = _.toPairs(valArray);

    var hashGoods = {};
    hashGoods.key = keysArray.join('(@)');
    hashGoods.val = valsArray.join('(@)');

    return hashGoods;
};

gcom.getMerchantGoodsKey = function(goods) {
    ...
    keyArray.type = goods.get('type');
    keyArray.sku = goods.get('sku');
    keyArray.updatedAt = new Date(goods.get('updatedAt')).getTime();
    keyArray.createdAt = new Date(goods.get('createdAt')).getTime();

    var keysArray = _.toPairs(keyArray);
    var goodskey = keysArray.join('(@)');
    return goodskey;
};

gcom.hashCategoryGoods = function(goods) {
    ...
    var goodsStruct = self.goodsStruct();
    keyArray.isdelete = 0;
    keyArray.tablename = gcom.getMysqlTable(TAB_C_GOODS,goods.get('parentid'));
    keyArray.parentid = goods.get('parentid');
    keyArray.storecategoryid = goods.get('storecategoryid');
    keyArray.goodsseries = goods.get('goodsseries');
    keyArray.status = goods.get('status');
...
gcom.getCategoryGoodsKey = function(goods) {
...
gcom.revertHashKey = function(hashkey) {
    var keyArray = hashkey.split('(@)')
    var keysArray = _.fromPairs(_.invokeMap(keyArray, String.prototype.split, ','));
    return keysArray;
}
