...
function policiesLayer(desc,req) {
    ...
    //访问标注
    if (req === -1) {
        sails.log.info("\n .....<" + desc + ">.....PoliciesIndex...." + gIndex + "........ \n");
    }else{
        sails.log.info("\n .....<" + desc + req.path + ">.....PoliciesIndex...." + gIndex + "........ \n");
    }

}

function policiesReset(desc,req) {
...
function policiesTheEnd(desc,req) {
...
function toQuotedString(str) {
  return '\'' + escapeQuotes(str.toString()) + '\'';
}
...
function isPassBuffer(val) {

    this._bufferArray = this._bufferArray || [];

    var isPass = false;
    var length = this._bufferArray.length;
    switch(length) {
        case 0:
            // isPass = true;
            this._bufferArray.push(val);
            break;
        case 1:
            isPass = (_.indexOf(this._bufferArray,val) > -1) ? false : true;
            if (isPass) {
                this._bufferArray.pop();
                this._bufferArray.push(val);
            };
            break;
        default:
            console("err setBuffer:  ",length);
    }
...
module.exports = {
    ...
    policiesReset:policiesReset,
    policiesLayer:policiesLayer,
    policiesTheEnd:policiesTheEnd,
    toQuotedString:toQuotedString,
    isPassBuffer:isPassBuffer,
};
