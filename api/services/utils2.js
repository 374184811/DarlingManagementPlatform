...
module.exports = {
    ...
    */
     ...
     * @没有返回参数
     */
    getWarnGroup: function (userid,cb) {

        console.log("readyWarnGroup ready.");
        var tableName = "seckillwarn" + ((new Date()).Format("yyyy"));
        async.auto({
            times:function(callback){
                var cmdStr = '/usr/bin/curl http://localhost:1338/Seckill/times';
                console.log('Seckill/times === >',cmdStr);
                exec(cmdStr, function(err,stdout,stderr) {
                    callback(err,JSON.parse(stdout).data);
                });
            },
            warn:['times',function(callback,data1){
                var i = 0,userwarns = [];
                var seckillingtimeArr = data1.times||[];
                seckillingtimeArr.forEach(function (time) {
                    var timeObj = {},g = 0;
                    var firstTime = (new Date(time.firsttime)).getTime();
                    var newDate = (new Date(time.firsttime)).Format('yyyy-MM-dd hh:mm:ss');

                    var nowStr = new Date().Format('yyyy-MM-dd hh:mm:ss');

                    var warnSql3 = 'SELECT COUNT(sku) AS num,sku FROM '+ tableName +' WHERE iscancel=1 AND time=\''+newDate+'\' AND time >\''+nowStr+'\' GROUP BY sku';
                    console.log(warnSql3);
                    Creator.query(warnSql3,function (err, result3) {
                        if(result3.length){
                            result3.forEach(function (skuObj,key) {
                                var warnSql2 = 'SELECT id,iscancel FROM '+ tableName +' WHERE sku=\''+skuObj['sku']+'\' AND  userid='+userid+' AND time=\''+newDate+'\' AND time >\''+nowStr+'\'';
                                console.log(warnSql2);
                                Creator.query(warnSql2,function (err, result4) {

                                    //不同时间段的数据处理
                                    var obj = skuObj;
                                    if(result4.length){
                                        obj.id = result4[0]['id']||0;
                                        obj.iscancel = result4[0]['iscancel']||0;
                                    }else {
                                        obj.id = 0;
                                        obj.iscancel = 0;
                                    }
                                    ...
                                    if(++g == result3.length){
                                        timeObj.firsttimestamp = firstTime;
                                        timeObj.warns = result3;
                                        userwarns.push(timeObj);

                                        if(++i == seckillingtimeArr.length){
                                            callback(err, userwarns);
                                        }
                                    }
                                });
                            });
            ...
            cb(err,list.warn);
            console.log('cb_tag1: The result of select warn is shown came out. check it out: ',list.warn.length);
        });
    },
};
