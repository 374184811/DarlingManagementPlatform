...
module.exports = {
    ...
    serviceGetDeliverInfo: function(req, res){
        ...
        var refundrnumber = reqData['refundrnumber']||'';
        var logisticsnumber = reqData['logisticsnumber']||'';

        var sqlStr1 = 'SELECT a.* FROM kuaidilog AS a WHERE a.deliverorder=\''+ logisticsnumber +'\'';
            sqlStr1 += ' AND payorder=\''+ ordernumber +'\' AND refundrnumber=\''+refundrnumber+'\'';
            console.log(sqlStr1);
    ...
    */
    ...
    merTakeDelivery: function(req, res) {
        ...
        if (data['type'] == 2) {
            ...
            var insertStr = 'INSERT INTO kuaidilog (`company`,`payorder`,`refundrnumber`,`deliverorder`,`tablenameofitem`,`saleflag`,`imgaddress`,`remark`,`dayinfo`,`status`,`detailbody`) ';
            insertStr += 'VALUES (\''+ data['logisticsid'] +'\',\''+ data['ordernumber'] +'\',\''+ data['refundrnumber']+ '\',\''+ data['logisticsnumber'] +'\',\'';
            insertStr += data['tablenameofitem'] +'\',\''+ data['saleflag'] +'\',\''+ data['imgaddress']+ '\',\'';
            insertStr += data['remark'] +'\',\''+ utility2.getCurday() +'\','+ 6 +',\''+ JSON.stringify(detailbody) +'\')';
            ...
            Kuaidilog.query(insertStr, function(err, results){
                ...
                //调用发货时候，修改子表订单状态函数
                if(data['saleflag'] == 2){
                    utiorder.updateSaleAfterDeliverStore(data['ordernumber'],data['logisticsnumber'],data['tablenameofitem'],data['refundrnumber']);
                }else{
            ...
            insertStr += 'VALUES (\''+ data['logisticsid'] +'\',\''+ data['ordernumber'] +'\',\''+ data['refundrnumber']+ '\',\''+ data['logisticsnumber'] +'\',\'';
            insertStr += data['tablenameofitem'] +'\',\''+ data['saleflag'] +'\',\''+ data['imgaddress']+ '\',\'';
            insertStr += data['remark'] +'\',\''+ utility2.getCurday() +'\','+ 7 +',\''+ JSON.stringify(detailbody) +'\')';
            console.log(insertStr);
            Kuaidilog.query(insertStr, function(err, results){
                if(err){
                    console.log("merTakeDelivery ==> ", err);
